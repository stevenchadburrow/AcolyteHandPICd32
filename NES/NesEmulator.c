...
// Compile:
// gcc -o NesEmulator.o NesEmulator.c -lglfw -lGL
...
#define CART_MAX 786448
unsigned char cart_rom[CART_MAX]; // largest cart is Kirby at 786448 bytes

unsigned char cpu_ram[2048]; // only cpu ram from 0x0000 to 0x07FF
unsigned char ppu_ram[2048]; // ppu ram from 0x2000 to 0x2FFF (halved, mirrored)
unsigned char prg_ram[8192]; // cpu ram from 0x6000 to 0x7FFF (if used)
unsigned char chr_ram[8192]; // ppu ram from 0x0000 to 0x1FFF (if used)
...
unsigned char nes_hack_top_hud = 0; // Duck Tales
unsigned char nes_hack_bottom_hud = 0; // Mario 3 and Kirby
unsigned char nes_hack_sprite_priority = 0; // Mario 3
unsigned char nes_hack_vertical_shift = 0; // Battletoads
...
unsigned char nes_burn(char *filename)
{
	...
	if (!input)
	{
		printf("Could not open cart rom!\n");
		return 0;
	}
	...
	for (unsigned long i=0; i<CART_MAX; i++)
	{
...
{
	screen_buffer[pos_y * 256 + pos_x] = color;
}
...
void drawScreen()
{
	...
	for (unsigned long j=0; j<240; j++)
	{
		...
		{
			glColor3f((float)((screen_buffer[j*256+i] & 0xE0) >> 5) / 8.0f,
				(float)((screen_buffer[j*256+i] & 0x1C) >> 2) / 8.0f,
				(float)((screen_buffer[j*256+i] & 0x03)) / 4.0f);
...
int main(const int argc, const char **argv)
{
	if (argc <= 1)
	{
		printf("Needs ROM filename as argument\n");
		return 0;
	}
	...
	window = glfwCreateWindow(open_window_x, open_window_y, "NesEmulator", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return 1;
