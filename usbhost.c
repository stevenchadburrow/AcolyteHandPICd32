...
void usbhost_initialize_endpoints(int address)
{
...
void usbhost_ep0_send_setup(unsigned char *buffer, unsigned long length)
{
...
void usbhost_ep0_send(unsigned char *buffer, unsigned long length)
{
	unsigned char *fifo = (unsigned char *)&USBFIFO0;
...
void usbhost_ep0_receive(unsigned char *buffer, unsigned long length)
{
...
unsigned int usbhost_ep0_receive_long(unsigned char *buffer, unsigned long length)
{
...
void usbhost_ep1_receive(unsigned char *buffer)
{
...
void USBHostSetup()
{
	...
	*((unsigned char*)&USBEOFRST + 0x3) = 0x00;
	...
	IPC33bits.USBIP = 6; // priority level 6
	...
	IPC33bits.USBDMAIS = 0; // sub-priority level 0
