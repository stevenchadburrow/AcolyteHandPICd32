			...
			if (bytes > 0)
			{
				...
				{	
					...
					{	
						...
						if (high_address >= 0x1D10 && high_address < 0x1D20)
						{
							SendChar('.');
...
void __attribute__((keep,address(0x9D100000))) TestingFunction()
{
	while (U3STAbits.UTXBF == 1) { }
	U3TXREG = (char)('?');
};
...
#pragma region name="user" origin=0x9D100000 size=0x00100000
#define USER __attribute__((region("user")))

volatile char testing_char;

void USER Testing()
{
	testing_char = '!';
	
	SendChar(testing_char);
}
...
unsigned int NVMWriteWord (unsigned long address, unsigned int data)
{
	unsigned int res;
	// Load data into NVMDATA register
	NVMDATA0 = data;
...
/*
	...
	unsigned int high_address = 0x0;
	unsigned int low_address = 0x0;
	unsigned int word = 0x0000;
		...
		while (bytes > 0)
		{
			...
			if (bytes > 0)
			{
				...
				{	
					...
					if (line[5] >= 'a') low_address += (unsigned int)((line[5] - 'a' + 10) * 0x0001);
					else low_address += (unsigned int)((line[5] - '0') * 0x0001);
					
					if (line[4] >= 'a') low_address += (unsigned int)((line[4] - 'a' + 10) * 0x0010);
					else low_address += (unsigned int)((line[4] - '0') * 0x0010);
					
					if (line[3] >= 'a') low_address += (unsigned int)((line[3] - 'a' + 10) * 0x0100);
					else low_address += (unsigned int)((line[3] - '0') * 0x0100);
					
					if (line[2] >= 'a') low_address += (unsigned int)((line[2] - 'a' + 10) * 0x1000);
					else low_address += (unsigned int)((line[2] - '0') * 0x1000);
					
					if (line[6] == '0' && line[7] == '0') // data
					{	
						for (unsigned char i=8; i<pos; i+=4)
						{
							word = 0x0000;
							
							if (line[i+3] >= 'a') word += (unsigned int)((line[i+3] - 'a' + 10) * 0x0001);
							else word += (unsigned int)((line[i+3] - '0') * 0x0001);

							if (line[i+2] >= 'a') word += (unsigned int)((line[i+2] - 'a' + 10) * 0x0010);
							else word += (unsigned int)((line[i+2] - '0') * 0x0010);

							if (line[i+1] >= 'a') word += (unsigned int)((line[i+1] - 'a' + 10) * 0x0100);
							else word += (unsigned int)((line[i+1] - '0') * 0x0100);

							if (line[i] >= 'a') word += (unsigned int)((line[i] - 'a' + 10) * 0x1000);
							else word += (unsigned int)((line[i] - '0') * 0x1000);
							
							if (line[i] >= 0x20 && line[i+1] >= 0x20 && line[i+2] >= 0x20 && line[i+3] >= 0x20)
							{
								if (high_address >= 0x1D10 && high_address < 0x1D20)
								{
									SendChar('.');
									
									NVMWriteWord((unsigned long)VirtToPhys((void *)((high_address << 16) + low_address)), word);
								}
							}
						}
					}
					...
					{	
						...
						if (line[11] >= 'a') high_address += (unsigned int)((line[11] - 'a' + 10) * 0x0001);
						else high_address += (unsigned int)((line[11] - '0') * 0x0001);

						if (line[10] >= 'a') high_address += (unsigned int)((line[10] - 'a' + 10) * 0x0010);
						else high_address += (unsigned int)((line[10] - '0') * 0x0010);

						if (line[9] >= 'a') high_address += (unsigned int)((line[9] - 'a' + 10) * 0x0100);
						else high_address += (unsigned int)((line[9] - '0') * 0x0100);

						if (line[8] >= 'a') high_address += (unsigned int)((line[8] - 'a' + 10) * 0x1000);
						else high_address += (unsigned int)((line[8] - '0') * 0x1000);
					}
					else
					{
...
/*
	...
	unsigned long storage[64];
	
	for (unsigned long i=0; i<64; i++)
	{
		storage[i] = 0x00000000;
	}
	
	for (unsigned long i=0; i<64; i++)
	{
		storage[i] = (*(long *)(0x9D040000+0x04*i));
		
		if (storage[i] == 0x0F408000)
		{
			storage[i] = 0x0F410000; // from 0x0F408000 to 0x0F410000
	...
	NVMErasePage(0x1D040000);
	
	for (unsigned long i=0; i<64; i+=4)
	{		
		NVMWriteQuadWord(0x1D040000+0x04*i, storage[i], storage[i+1], storage[i+2], storage[i+3]);
	}
	
	asm("jal 0x9D040000"); // hex 0x0F410000
	asm("nop");

void __attribute__((keep,address(0x9D040000))) AnotherFunction()
{
	U3TXREG = (char)('A');
	while (U3STAbits.UTXBF == 1) { }
	
	asm("jal 0x9D020000"); // hex 0x0F408000
	asm("nop");
}
*/
